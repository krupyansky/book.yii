...
class BookController extends AppAdminController
{
    ...
    {
        $this->findModel($id)->unlinkAll('bookAuthors', true);
        $this->findModel($id)->unlinkAll('bookCategories', true);
     ...
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id, $categories = false)
    {
        if ($categories) {
            $query = Book::find()->with('categories')->andFilterWhere(['book.id' => $id])->one();
        } else {
            $query = Book::findOne($id);
        }
        if (($model = $query) !== null) {
            return $model;
        }
