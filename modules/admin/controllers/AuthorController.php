...
use app\modules\admin\models\BookAuthor;
...
class AuthorController extends AppAdminController
{
    ...
    {
        $authors = BookAuthor::find()->where(['author_id' => $id])->count();
        if ($authors) {
            Yii::$app->session->setFlash('error', 'Ошибка! К удаляемому автору привязаны книги.');
        } else {
